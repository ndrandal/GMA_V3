...
// calls this once per subscription and puts the result in `tree::Deps::strand`.
// It is D3's named mint site, the one every real subscription goes through.
//
// It is a named function rather than two inline lines because those two lines
// could be DELETED WITH A GREEN SUITE. ENC-1005 mutation-tested its own work
// ten ways and published the two that reddened nothing — M7 (delete this mint)
// and M10 (drop its null-pool guard) — both because `tree::buildForRequest`'s
// backstop quietly mints a replacement that no assertion could tell apart.
// Extracting the mint gives the guard a direct test; tagging the strand it
// returns (`rt::Strand::origin()`) gives the CALL a direct test. Together they
// are ENC-1338's gate: `tests/ws/SubscriptionStrandMintTest.cpp`.
//
// Returns null when `pool` is null, and that is the guard M10 removes: with no
// executor there is nothing to serialise, delivery is already inline and
// already in order, and a pool-less Strand would additionally make the
// `Listener` answer `deliversOnOwnExecutor()` — so `Dispatcher` would go inline
// too and the entire DAG compute would run on the WebSocket read thread. The
// backstop has always had this guard; before ENC-1005 the production site did
// not.
std::shared_ptr<gma::rt::Strand> mintSubscriptionStrand(gma::rt::ThreadPool* pool);

// The `rt::Strand::origin()` tag `mintSubscriptionStrand` stamps. Compare
// against this rather than re-spelling the literal — the point of the tag is
// that exactly ONE place in the engine can produce it.
